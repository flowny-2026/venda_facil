...
// ============================================
export const generateReceiptPDF = (sale, company, size = '80mm') => {
    ...
    // Função: Duas colunas
    const addTwoColumns = (left, right, fontSize = 9, bold = false) => {
        doc.setFontSize(fontSize);
        doc.setFont('helvetica', bold ? 'bold' : 'normal');
...
// ============================================
export const printReceipt = (sale, company, size = '80mm') => {
...
// FUNÇÃO: BAIXAR PDF
// ============================================
export const downloadReceipt = (sale, company, size = '80mm') => {
    const doc = generateReceiptPDF(sale, company, size);
...
// ============================================
export const shareReceiptWhatsApp = (sale, company, phoneNumber) => {
    const message = `
*${company.name}*
...
*PRODUTOS:*
${sale.items.map(item => `${item.product_name}\n${item.quantity}x R$ ${item.unit_price.toFixed(2)} = R$ ${item.total_price.toFixed(2)}`).join('\n\n')}
...
// ============================================
export const getReceiptBlob = (sale, company, size = '80mm') => {
    const doc = generateReceiptPDF(sale, company, size);
