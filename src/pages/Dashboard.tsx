...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    loadData();
  }, [user, authLoading, useSupabase]);

  const loadData = async () => {
    setIsLoading(true);
    console.log('🔄 Iniciando carregamento de dados...');
    console.log('📊 useSupabase:', useSupabase, 'user:', !!user);
    
    try {
      if (useSupabase && user) {
        console.log('🔗 Carregando dados do Supabase...');
        
        // Carregar vendas reais do Supabase
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-100">Painel de Vendas</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard
          ...
          hint={`${totalOrders} pedidos no período`}
        />
        <KpiCard
          title="Pedidos"
